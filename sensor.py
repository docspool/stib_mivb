...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    if config.get(CONF_MESSAGE_LANG):
...
class StibMivbSensor(SensorEntity):
    ...
    def __init__(self, api, stop_id, line_ids, lang, message_lang):
        ...
        self._attributes = {"stop_id": self.stop_id, ATTR_ATTRIBUTION: ATTRIBUTION}
    ...
    async def async_update(self):
        """Get the latest data from the STIB-MIVB API."""
        ...
        self._attr_name = f"{self._stop_name} line {' '.join(self.line_ids)}"
        ...
        try:
            ...
            _LOGGER.error(
                ...
                self.stop_id, HttpException, getattr(HttpException, "text", ""),
            )
            return
        ...
        try:
            ...
            _LOGGER.error(
                ...
                self.line_ids, HttpException, getattr(HttpException, "text", ""),
            )
        ...
        for passing_time in waiting_time_response["points"][0].get("passingTimes", []):
            for line_id in self.line_ids:
                if passing_time.get("lineId") != line_id:
                    continue

                next_passage = {}
                try:
                    next_passage["next_passing_time"] = passing_time["expectedArrivalTime"]
                    next_passage["next_passing_destination"] = passing_time["destination"][self.lang]
                    tmp = pytz.utc.normalize(datetime.datetime.fromisoformat(next_passage["next_passing_time"]))
                    next_passage["waiting_time"] = round((tmp - now).total_seconds() / 60)
                except KeyError:
                    _LOGGER.debug(
                    ...
                    try:
                        next_passage["next_passing_message"] = passing_time["message"][self.lang]
                    except KeyError:
                        _LOGGER.error("No arrivaltime and no message for line %s", line_id)
                ...
                for message in messages_response.get("messages", []):
                    for line in message.get("lines", []):
                        if line.get("id") == line_id:
                            for stop in message.get("points", []):
                                if stop.get("id") == self.stop_id:
                                    try:
                                        messages.append(message["content"][0]["text"][0][self.lang])
                                    except Exception:  # structure variable suivant l'API
                                        pass

                type_id = await self.api.get_line_type(line_id)
                next_passage["messages"] = messages
                next_passage["line_type"] = TYPES.get(type_id, "bus")
                next_passage["line_color"] = await self.api.get_line_color(line_id)
                next_passage["line_text_color"] = await self.api.get_line_text_color(line_id)

                next_passages.append(next_passage)

        self._next_passages = sorted(next_passages, key=lambda i: i.get("waiting_time", math.inf))

        if not self._next_passages:
            self._native_value = None
            self.__icon = "mdi:bus-alert"
            return

        self._attributes["line_name"] = self._next_passages[0].get("line_name")
        self._attributes["line_type"] = self._next_passages[0].get("line_type")
        self._attributes["line_color"] = self._next_passages[0].get("line_color")
        self._attributes["line_text_color"] = self._next_passages[0].get("line_text_color")
        self._attributes["messages"] = self._next_passages[0].get("messages", [])
        self._attributes["next_passages"] = self._next_passages

        if self._next_passages[0].get("next_passing_time"):
            # Device class TIMESTAMP -> Home Assistant s'attend à un datetime ou une ISO-8601 UTC
            self._native_value = self._next_passages[0]["next_passing_time"]
            self.__icon = f"mdi:{self._next_passages[0]['line_type']}"
        else:
            # Pas d'heure → message d'info
            self._native_value = self._next_passages[0].get("next_passing_message")
            self.__icon = "mdi:bus-alert"

    @property
    def native_value(self):
        """Return the sensor native value (timestamp or message)."""
